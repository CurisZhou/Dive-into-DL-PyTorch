...
def train_and_predict_rnn(rnn, get_params, init_rnn_state, num_hiddens,
    ...
    for epoch in range(num_epochs):
        ...
        for X, Y in data_iter:
            ...
            # outputs列表内num_steps个(batch_size, vocab_size)的张量,沿着dim=0拼接之后变为形状为(num_steps * batch_size, vocab_size)的张量
            outputs = torch.cat(outputs, dim=0)
            # Y的形状是(batch_size, num_steps)，转置后再变成长度为num_steps * batch_size 的向量，这样跟输出的行一一对应
